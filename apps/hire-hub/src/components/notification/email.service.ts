...
export class EmailService {
	...
	/**
	 * [SERVICE] Send verification email with code
	 * @param email - Recipient email address
	 * @param code - 6-digit verification code
	 * @param firstName - User's first name for personalization
	 */
	public async sendVerificationEmail(email: string, code: string, firstName?: string): Promise<void> {
	...
	/**
	 * [SERVICE] Send password reset email
	 * @param email - Recipient email address
	...
	public async sendTwoFactorAuthCodeEmail(email: string, code: string): Promise<void> {
		const mailOptions = {
			from: EnvUtil.getEmailFrom(),
			to: email,
			subject: 'Your Two-Factor Authentication Code - HireHub',
			html: `
				<!DOCTYPE html>
				<html>
				<head>
		...
		try {
			await this.transporter.sendMail(mailOptions);
			this.logger.log(`Two-factor authentication code email sent to ${email}`);
		} catch (error) {
			this.logger.error(`Failed to send two-factor authentication code email to ${email}`, error);
			throw new Error('Failed to send two-factor authentication code email');
		}
	}
}
